...
- **Hasil aktual:** endpoint auth, guard namespace privat, sesi PostgreSQL, throttle atomik dan job cleanup tersedia; lihat [panduan auth](./auth.md) dan [bukti T-06](./evidence/t06-auth.md). Tes HTTPS/DB dan regresi sudah ditulis tetapi belum dijalankan sesuai instruksi pemilik; status selesai menunggu bukti perilaku.
